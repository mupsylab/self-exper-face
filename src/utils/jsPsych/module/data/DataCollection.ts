import { TrialResult, TrialResults } from "../../timeline";
import { deepCopy } from "../utils";
import { DataColumn } from "./DataColumn";
import { JSON2CSV, saveTextToFile } from "./utils";

export class DataCollection {
  private trials: TrialResults;

  constructor(data: TrialResults = []) {
    this.trials = data;
  }

  push(new_data: TrialResult) {
  ...
  addToAll(properties: TrialResult) {
  ...
  addToLast(properties: TrialResult) {
  ...
  filter(filters: TrialResult | TrialResults) {
    // [{p1: v1, p2:v2}, {p1:v2}]
    // {p1: v1}
    let f: TrialResults;
  ...
  filterCustom(fn: (value: TrialResult, index: number, array: TrialResults) => value is TrialResult) {
